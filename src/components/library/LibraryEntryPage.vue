...
export default {
    ...
    created() {
                this.get(`/users/${this.userID}/bookshelves/${this.$route.params.id}`).then((response)=>{
                    this.shelf = response.data;                 
            }); 
                this.get(`/users/${this.userID}/bookshelves/${this.$route.params.id}/volumes`).then((response)=>{
